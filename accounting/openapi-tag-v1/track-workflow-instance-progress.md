...
| **Canonical Owner** | TBD |
| **Primary User / Actor** | Finance User, Operations User (authenticated caller; typically the client app for a user in onboarding or company workflows) |
| **Business Outcome** | Users see how far a guided workflow has progressed and which section and task are active so they can resume work or explain status without guessing. |
| **Entry Point / Surface** | Sleek Back HTTP API: `GET /workflow-instances/:workflowInstanceId/progress` (from `controllers/workflow-controller.js`, mounted at `/` via `app-router` glob); requires `userService.authMiddleware`. Related OpenAPI surface groups workflow routes under tags `workflow` and `v1` in `swagger.yml` (same broad v1 workflow family; this path may not be listed verbatim in the spec). |
...
| **Disposition** | Unknown |
| **Open Questions** | If every task is non-`available` (e.g. all completed) or presentation order has no `available` task, `section` and `task` are omitted or undefined in the JSON; `percentage` divides by `counts.total`—if zero tasks, percentage is NaN. Confirm intended API contract. Whether this route is or should be listed in OpenAPI alongside `/workflow-instances`. |
| **Reviewer** | |
| **Review Status** | Draft |
